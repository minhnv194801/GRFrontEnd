...
function Read() {
  ...
  const sendReport = (e) => {
    const postReport = async () => {
      ...
      try {
        const response = await fetch(process.env.API_ENDPOINT+'/report/' + chapterId, {
          method: 'POST',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let newSessionkey = await refresh()
        const response = await fetch(process.env.API_ENDPOINT+'/read/' + chapterId, {
          method: 'GET',
          credentials: 'same-origin',
          headers: {
            'Authorization': newSessionkey ? newSessionkey : sessionkey,
    ...
    const fetchChapterData = async () => {
      let newSessionkey = await refresh()
      try {
        const response = await fetch(process.env.API_ENDPOINT+'/read/' + chapterId + '/chapterlist', {
          method: 'GET',
          credentials: 'same-origin',
          headers: {
            'Authorization': newSessionkey ? newSessionkey : sessionkey,
